 ...
 * 크롤링 Job 스케줄러.
 *
 * <p>Job 2 (가격 데이터 수집)를 매일 새벽 3시에 자동 실행한다.
...
public class CrawlerScheduler {
    ...
    /**
     * 매일 새벽 3시 가격 데이터 수집 Job 자동 실행.
     * cron 표현식: 초 분 시 일 월 요일
     */
    @Scheduled(cron = "0 0 3 * * *")
    public void runDailyCrawl() {
        log.info("=== 일별 가격 수집 스케줄 실행 시작 ===");
        try {
            JobParameters params = new JobParametersBuilder()
                    .addLong("timestamp", System.currentTimeMillis())
                    .toJobParameters();
            jobLauncher.run(priceCrawlJob, params);
            log.info("=== 일별 가격 수집 스케줄 실행 완료 ===");
        } catch (Exception e) {
            log.error("일별 가격 수집 스케줄 실행 실패: {}", e.getMessage(), e);
        }
    }
}
