...
> **최종 업데이트**: 2026-04-21 (Phase 3 완료: 유저 프로필·등급 API, 신고 시스템 + 키워드 감지, 관리자 신고/차단/메시지 API, 단위 테스트 51개 통과)
...
| 키워드 감지 연동 (`KeywordDetectionService`) | ❌ 없음 | sendMessage에 TODO 존재 (1-6 연계) |
| 채팅 아카이브 배치 Job (6개월 경과) | ❌ 없음 | Phase 5 범위 |
| 채팅 영구 삭제 배치 Job (2년 경과) | ❌ 없음 | Phase 5 범위 |
| 단위 테스트 15개 (`ChatServiceTest`) | ✅ 통과 | createChatRoom·sendMessage·posterConfirm·counterpartyConfirm 4그룹 |
...
| 내 정보 조회 (`GET /api/users/me`) | ❌ 없음 | 등급·호봉 포함 |
| 내 등록글 목록 (`GET /api/users/me/listings`) | ❌ 없음 | |
| 회원 탈퇴 (`DELETE /api/users/me`) | ❌ 없음 | softDelete |
| `ExpGradeCalculator` 단위 테스트 | ❌ 없음 | 각 등급 경계값 검증 |
| 기타 단위 테스트 | ❌ 없음 | |

**100% 조건**: `ExpGradeCalculator` 등급·호봉 경계값 테스트 + 내 정보 조회·탈퇴·차단 상태 접근 거부 테스트 통과
...
| `MercenaryMaterial` 재구조화 | ✅ 구현 | `resultMercenary` / `materialMercenary`(nullable) / `materialItemKey`(nullable) |
| `MercenaryListTasklet` gerniverse JSON-LD 파싱 전환 | ✅ 구현 | geota CSR → gerniverse `/mercenary` ItemList JSON-LD |
| `MercenaryDetailReader` `crawledAt IS NULL` 조건 | ✅ 구현 | 기존 `imageUrl IS NULL` → `crawledAt IS NULL` |
| `MercenaryDetailWriter` 스탯·재료 재적재 | ✅ 구현 | MercenaryStat delete-reinsert, 아이템/용병 재료 분기 처리 |
| `GerniverseParser.parseMercenaryList()` 추가 | ✅ 구현 | JSON-LD ItemList 파싱 |
| `GerniverseParser.parseMercenary()` 신규 구조 반영 | ✅ 구현 | `MercenaryStatEntry` / `MaterialEntry` 재설계 |
...
| GemGrade 영문 코드 전환 | ⚠️ 보류 | Flyway 시드 설계 시 함께 처리 권장 |
| `GerniverseParser` 파싱 단위 테스트 (HTML fixture) | ❌ 없음 | 아이템·용병 파싱 모두 포함 |
| 통합 테스트 (Mock HTML 파싱 검증) | ❌ 없음 | |

**100% 조건**: `GerniverseParser` 파싱 단위 테스트 (고정 HTML fixture — 아이템·용병 목록/상세) + 장비 플래그 반영 + 테스트 통과
...
1-6.  신고 시스템 + 키워드 감지        █████░░░░░░░░░░░░░░░░░  25%  ← 엔티티·레포 완료
1-7.  시세 조회                        ██████████████████████ 100%  ← 완료
1-8.  유저 프로필·등급                 █████░░░░░░░░░░░░░░░░░  25%  ← 등급 엔티티·유틸 완료
...
16. **2-5 관리자 기능** — 신고 처리·사용자 차단·등록글 숨김 API (1-6 완료 후)